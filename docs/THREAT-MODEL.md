...
| **M3-B Commerce-over-Chat (capture/linkage + auto-reply + consent + RBAC cost-split + PII from chat)** | **APPROVED-WITH-CONDITIONS → 9 conditions VERIFIED IN CODE (bukani-qa 2026-06-25); residual R1 documented (GA blocker)** (§8) | Design-gate cleared at contract-freeze (2026-06-23); **build complete / QA APPROVED (`feat/m3b-backend`)**; live PII send re-gated at cutover (EA-ADR-015, E1–E4) |
...
## 8. M3-B Commerce-over-Chat — STRIDE (gate: design-gate before M3-B contract-freeze)

> **Commissioned by:** bukani-architect (M3-B contract-freeze dependency, brief §8.4 / contract §12).
> **Date:** 2026-06-23. **Risk class:** **high** (PII capture from an external channel into the commerce core + multi-tenant linkage + a new automated send path + the live-cutover boundary).
> **Data classes touched:** PII (WhatsApp customer phone `waContactId`, `Message.body`-derived context, `Customer` phone), money (ZAR-cents order/line totals), auth/RBAC (cost-split, new `whatsapp:manage_autoreply`). **External integration:** 360dialog (consumed via frozen M3-A seams; M3-B adds no new edge).

**Scope note:** M3-B adds **no public/unauthenticated network edge** — all 7 new/changed routes sit behind `app.authenticate` + RBAC under `/v1/businesses/:businessId/*`. The unauthenticated webhook edge, signature-verify-before-parse, provider-id idempotency, server-side number→tenant routing, and fast-ack-then-async remain **M3-A controls** (§7), carried forward here (Carried-Condition C9) because M3-B is the milestone that turns inbound events into orders/stock/PII and so re-confirms the end-to-end replay/idempotency chain.

### Asset inventory
- **`Conversation` ↔ `Order` linkage (`Order.conversationId`)** — sensitivity: high (cross-tenant link = confused-deputy) — owner: backend (capture path).
- **`Customer` created from `waContactId`** (phone PII) — sensitivity: high (POPIA personal information) — owner: tenant (`businessId`); `consentId` nullable.
- **`WhatsAppAutoReplyRule` config** — sensitivity: medium (controls automated paid sends) — owner: `MERCHANT_OWNER` only.
- **The consent enforcement point (`assertConsentGranted`)** — sensitivity: high (POPIA + WhatsApp policy gate on every send) — owner: bukani-security (policy), backend (call).
- **`costPriceCents` / margin / financial dashboard fields** — sensitivity: high (RBAC cost-split; must stay owner-only) — owner: backend.
- **Sync `payload` (`z.record(z.unknown())`)** — sensitivity: high (unvalidated passthrough now carrying `channel`/`conversationId` for offline capture) — owner: backend (sync service).
- **Per-tenant WhatsApp conversation cost** — sensitivity: medium (each send/conversation costs money; auto-reply automates it) — owner: founder (budget ceiling OPEN).

### Verified against code (the architect's two flags + two added)
1. **CONFIRMED — `POST /orders` body does not expose `channel`/`conversationId`.** `CreateOrderBody` (`server/src/routes/v1/commerce.routes.ts:70`) lacks both; the route never passes `channel` (handler `:340`), so it writes `IN_PERSON` only. The **service** `createOrder` (`server/src/services/order.service.ts:16,77`) *does* accept/write `channel`. M3-B's additive body fields + `Order.conversationId` column are real, not "already there." Back-compatible.
2. **CONFIRMED — `assertConsentGranted` is business-scoped, not customer-scoped.** `server/src/services/whatsapp-send.service.ts:38-62` resolves `consent.findFirst({ where: { businessId, purpose, status } })` with **no customer identity**; the revocation read (`revocations … take: 1`) is the latest revocation **of the business-level grant**, not of a specific customer. Per-customer revocation (S6/AC3) **cannot** be honored today. The `Consent`/`ConsentRevocation` models (`schema.prisma:175-200`) carry **no WhatsApp-customer subject reference** — a revocation is global to a `Consent` row, and the only customer link is `Customer.consentId`, which **stays nullable in M3-B**. So the §6.1 "customer-aware" branch is **structurally inert** unless a per-customer `Consent` row exists. (See I/E rows + Conditions 4 & 5; residual R1.)
3. **NEW FINDING — `createOrder` does not tenant-validate `customerId`.** `order.service.ts:75` writes `customerId: input.customerId ?? null` with **no** `customer.businessId === input.businessId` check (product lines *are* scoped at `:55`; the customer is not). A caller with `order:write` on tenant A can link tenant B's `Customer` to an A-order (cross-tenant read-by-reference / pollution). Pre-existing M2 gap that the M3-B capture surface **widens** (capture actively resolves/links customers). Closed under Condition 1.
4. **NEW FINDING — sync `payload` is unvalidated (`z.record(z.unknown())`, `commerce.routes.ts:108`).** Offline capture (contract §4.1b) puts `channel`/`conversationId` into this passthrough. Without a typed sub-schema, an offline op can smuggle arbitrary fields into `createOrder`. Condition 8 requires the sync order payload to be validated by the **same** Zod schema as the online body before reaching the service.

### STRIDE
| Threat | Vector | Likelihood | Impact | Mitigation (existing M3-A/M2 + **required new**) | Residual risk | Accepted? |
|---|---|---|---|---|---|---|
| **S**poofing | Caller forges a `conversationId` / `customerId` from another tenant on `POST /orders` or via the offline `sync` order op to attach foreign context to their own order | Medium | High (cross-tenant linkage / confused deputy) | Existing: JWT auth + `order:write`/`sync:write`; `businessId` server-resolved; M3-A send path already checks `conversation.businessId === businessId` (`whatsapp-send.service.ts:82`). **New: capture MUST load the `Conversation` and assert `conversation.businessId === route businessId` before writing `Order.conversationId` (Cond 1); MUST assert `customer.businessId === businessId` before linking `customerId` (Cond 1 — closes finding #3); offline `sync` payload validated identically (Cond 8).** | Low (with conditions) | **Y** — only if Conds 1 & 8 baked in |
| **T**ampering | Replayed/duplicated capture double-applies a SALE (double stock decrement / duplicate order); offline LWW clobbers; unvalidated sync payload mutates order fields | Medium | High (stock/financial integrity) | Existing & verified: `@@unique([businessId, clientId])` on `Order` short-circuits to `duplicate:true` (`order.service.ts:31-35`); deterministic `StockMovement.clientId = "<orderId>:sale:<productId>"` (`:107`) makes SALE idempotent; append-only ledger is convergent; LWW-on-`occurredAt` (ADR-INY-016). M3-A provider-id dedup on inbound `Message`. **New: no new idempotency mechanism (good); typed sync order payload (Cond 8); re-confirm the inbound→capture replay chain end-to-end (Carried C9).** | Low | **Y** |
| **R**epudiation | "I never captured that order / never sent that auto-reply"; disputed consent refusal | Low–Medium | Medium | Existing: `(order,CREATE)`, `(customer,CREATE)`, `(stock_movement,CREATE)`, `(whatsapp_message,SEND)` audit tuples (masked). **New: `(whatsapp_autoreply_rule, CREATE/UPDATE/DELETE)` for config; `(whatsapp_autoreply, FIRE)` AND `(whatsapp_autoreply, SUPPRESSED)` so every fire/suppress (by window/consent/cooldown/loop-guard) is observable (Cond 6 — the audit hook the gate relies on to *prove* suppression).** | Low | **Y** |
| **I**nfo disclosure | (a) Reading another tenant's `Conversation`/`Message` via the inbox; (b) raw msisdn/`Message.body` leaking into logs/audit on customer-create; (c) **cost/margin leaking to `MERCHANT_STAFF`** via the picker, catalog share, or captured-order view; (d) `costPriceCents` logic shipped to the client | Medium | High (POPIA PII + RBAC cost-split) | Existing & verified: M3-A inbox reads fail-closed on `conversation.businessId !== businessId` → 404 (`whatsapp.routes.ts:184,203`); chassis `logger`+`pii-mask`; catalog share is **server-composed sell-price-only** (no client price logic, route never reads cost). **New: every customer-create/order-create log line + audit `changes` MUST be pii-masked — no raw `waContactId`/`body` (Cond 3); the catalog-share composer + capture picker MUST NOT read or return `costPriceCents` and the staff order view MUST hide (not zero) cost/margin/financial (Cond 2).** | Low (with conditions) | **Y** — only if Conds 2 & 3 baked in |
| **D**enial of service | **Auto-reply loop / cost-DoS:** an auto-reply fires on an OUTBOUND/echo/STATUS event, or two auto-responders ping-pong, or a status-callback storm drives unbounded **paid** WhatsApp sends; oversized sync batch | Medium | Medium–High (unbounded spend — WhatsApp conversations cost money) | Existing & verified: inbound persists `direction:'INBOUND'` + typed `MessageType` (`whatsapp-ingest.service.ts:191`); status callbacks are handled as status, not messages (no Message row) → cannot trigger; sync ≤100 ops + partial success. **New: auto-reply evaluator MUST fire ONLY when the just-persisted Message is `direction=INBOUND` AND `type ∈ {TEXT, INTERACTIVE}` — NEVER OUTBOUND, NEVER STATUS, NEVER echoes; MUST enforce `cooldownMinutes`/once-per-period derived from the OUTBOUND-auto-reply ledger of the same `trigger` (Cond 7); MUST send through `sendWhatsAppMessage()` so the window/`enabled` gates apply (Cond 6).** **ESCALATE: a per-tenant WhatsApp conversation-cost ceiling + kill switch is OPEN (founder/EA) — not solvable in M3-B code (Escalation E1).** | **Medium** | **Conditional** — code path accepted iff Cond 6 & 7; the *cost ceiling* is an accepted residual **only** with E1 escalated |
| **E**levation | (a) `MERCHANT_STAFF` reconfiguring auto-replies (owner-only boundary); (b) **consent-bypass**: a `SHARE_CATALOG`/`SEND_TEXT` auto-reply, or a status/catalog send, reaching a customer who revoked or while LIVE-disabled, because the consent point is business-scoped and the per-customer branch is inert (finding #2); (c) **AI creep**: a "smart" reply pre-empting the M5 EA-ADR-012 autonomy gate | Medium | High (POPIA/WhatsApp-policy violation; autonomy-gate bypass) | Existing & verified: new `whatsapp:manage_autoreply` owner-only, read via `whatsapp:read`; all sends route through `sendWhatsAppMessage()` → `enabled` gate (`whatsapp-send.service.ts:87`) + `assertConsentGranted` (`:92`); `AI_AGENT` read-only, untouched. **New: every M3-B send (S2/S4/S5/S7) MUST pass through the single `sendWhatsAppMessage()`/`assertConsentGranted` point — no side-door (Cond 4); the consent point MUST be made customer-aware AND the model gap closed so per-customer revocation is honorable when a customer-scoped grant exists, else the default-deny marketing branch holds (Cond 5); auto-reply module MUST have ZERO `lib/ai.js` references, enforced by a CI grep assertion (Cond 6c).** | Low–Medium | **Conditional** — accepted iff Conds 4, 5, 6c baked in; the **structural consent gap (finding #2) is the one item that must be explicitly resolved or formally accepted** (residual R1) |

### Re-evaluation triggers
- **Live-number cutover** (sandbox → production PII): re-open under the EA-ADR-015 360dialog DPA + EU-pin + bukani-compliance risk assessment, and the responsible-party consent ruling (POPIA §7a/§8.1). **No production WhatsApp PII until those clear** — M3-B ships DARK behind `WhatsAppChannel.enabled`.
- **The customer-directory / WhatsApp responsible-party ruling lands** → the §6 branch policy changes (one function), and per-customer consent rows may begin populating `Customer.consentId` → re-confirm finding #2 is fully wired, not just default-deny.
- **A per-tenant cost ceiling is set** (founder) → wire it as a hard pre-send gate + kill switch and re-confirm the auto-reply DoS row.
- **If marketing/broadcast is ever scoped** (out of M3-B) → full re-model (heavier consent + cost + abuse).
- **If any auto-reply action is ever made generative / intent-detecting** → this becomes the EA-ADR-012 AI-agent surface and inherits the M5 hard STRIDE gate.

### Verdict
**APPROVED-WITH-CONDITIONS (build verified by bukani-qa 2026-06-25, branch `feat/m3b-backend`).** Originally cleared at contract-freeze provided the architect baked the 9 conditions into the frozen contract; **all 9 are now verified in code** (migration `20260623124013_m3b_commerce_over_chat`). No unmitigated high-severity threat remains **for the sandbox slice with zero production PII**. The two **Conditional** rows (auto-reply DoS, consent-bypass/elevation) and the **structural consent gap (finding #2)** are resolved by the implemented conditions; the **WhatsApp cost ceiling** (E1) and the **responsible-party ruling** (E2) remain **escalated**, not solved in code.

### Conditions baked into the frozen M3-B contract — all VERIFIED in code (2026-06-25)
1. **Tenant-isolation on capture (closes findings #1 & #3).** Every order-capture handler (online `POST /orders` and the offline `sync` order op) MUST, before writing `Order.conversationId` or `Order.customerId`: load the `Conversation` and reject 403/404 unless `conversation.businessId === route businessId`; and load the `Customer` and reject unless `customer.businessId === route businessId`. Never write a cross-tenant link.
2. **RBAC cost-split on every M3-B commerce surface (hide, not zero).** The capture catalog-picker, the `share-catalog` composer, and the captured-order view returned to a `MERCHANT_STAFF` caller MUST omit `costPriceCents`, margin, and financial-dashboard fields by omission (not `0`). The `share-catalog` route MUST NOT read `costPriceCents` at all.
3. **PII masking on the new capture path.** Customer-create-from-`waContactId`, the `(customer,CREATE)`/`(order,CREATE)` audit `changes`, and every log line on the capture/auto-reply path MUST be pii-masked. `Customer.consentId` stays nullable.
4. **Single send choke-point — no auto-reply side-door.** All four M3-B send paths (S2/S4/S5/S7) MUST flow through `sendWhatsAppMessage()` → `assertConsentGranted` + window-selection + `WhatsAppChannel.enabled` gate.
5. **Make consent customer-aware AND close the structural gap (finding #2).** (a) extend `assertConsentGranted(businessId, sendClass, isTemplate, ctx)`; (b) specify the data model scoping a revocation to a WhatsApp customer; (c) preserve default-deny marketing + transactional-in-open-window allow when no customer-scoped grant exists; (d) the ruling changes only the branch policy in this one function. If (b) is deferred, the contract MUST state per-customer revocation (S6/AC3) is NOT met in M3-B and ships default-deny-only (residual R1 — conscious documented acceptance).
6. **Auto-reply gate + provably non-AI + observable suppression.** (a) send only via `sendWhatsAppMessage()`; (b) emit `(whatsapp_autoreply, FIRE)` and `(whatsapp_autoreply, SUPPRESSED)` masked audit; (c) ZERO `lib/ai.js` in the auto-reply path — CI grep assertion.
7. **Loop / cost-DoS prevention.** Fire ONLY when the just-persisted `Message` is `direction = INBOUND` AND `type ∈ {TEXT, INTERACTIVE}`; enforce `cooldownMinutes`/once-per-period. **As built (`ceee30e`), cooldown is keyed by `ruleId`** (the prior `(whatsapp_autoreply, FIRE)` audit entry for the same `ruleId` on the conversation within `cooldownMinutes`) — stricter than the contract's per-`trigger` wording.
8. **Validate the offline sync order payload** with the same typed schema as the online `POST /orders` body; no unvalidated field reaches `createOrder`; partial-success preserved.
9. **Carry the M3-A conditions forward** (signature-verify-before-parse, provider-id idempotency, server-side number→tenant routing, pii-masked logging, fast-ack-then-async) and re-confirm the inbound→capture replay/idempotency chain end-to-end.

### Residual risks explicitly accepted
- **R1 — Per-customer consent revocation (S6/AC3) may not be functionally met in M3-B.** Accepted ONLY as default-deny-marketing for the sandbox slice, and ONLY if documented per Cond 5. Re-eval: live cutover OR the responsible-party ruling — at which point 5(b) is a hard GA blocker.
- **R2 — `Customer.consentId` nullable.** Accepted: a transactional sale is a distinct lawful basis from optional messaging (S6/AC5).
- **R3 — Live-send DoS surface.** Accepted for sandbox; bounded by Cond 7 + Cloudflare edge. Live-relevant at cutover.

### Items to ESCALATE (not solvable in M3-B code)
- **E1 — WhatsApp conversation cost ceiling + kill switch → founder / bukani-enterprise-architect.** Cond 7 bounds loops, not aggregate spend. Live-cutover gate (analogous to the EA-ADR-011 R3,000/mo AI ceiling).
- **E2 — Customer-directory / WhatsApp responsible-party ruling → bukani-compliance (GA-gating).** Governs the §6 branch policy. M3-B builds default-deny; does not invent the ruling.
- **E3 — 360dialog sub-processor DPA + EU-pin + risk assessment → bukani-compliance (EA-ADR-015 extension, GA-gating live).** Live flow stays DARK behind `WhatsAppChannel.enabled`; no production PII until cleared.
- **E4 — Message/Conversation→Order/Customer retention period → bukani-compliance.** The M3-A `whatsapp.message.retentionDays` Setting is the seam (unset → no purge). Do not hard-code.

**Gate: APPROVED-WITH-CONDITIONS — CLEARED. Conditions 1-9 are folded into the frozen contract AND verified in code (bukani-qa APPROVED 2026-06-25, branch `feat/m3b-backend`); R1 is consciously documented per Condition 5 and remains a GA blocker (E2-gated). Awaiting SRE/devops sign-off + merge to main; live cutover re-gates under EA-ADR-015 (E3).**
