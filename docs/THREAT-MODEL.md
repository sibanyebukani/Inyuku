...
| **M3-B Commerce-over-Chat (capture/linkage + auto-reply + consent + RBAC cost-split + PII from chat)** | **APPROVED-WITH-CONDITIONS → 9 conditions BAKED into the frozen contract; residual R1 documented** (§8) | Design-gate cleared at contract-freeze (2026-06-23); build pending; live PII send re-gated at cutover (EA-ADR-015, E1–E4) |
...
### Verdict
**APPROVED-WITH-CONDITIONS** — proceed to M3-B contract-freeze provided the architect bakes the 9 conditions below into the frozen contract. No unmitigated high-severity threat remains **for the sandbox slice with zero production PII**. The two **Conditional** rows (auto-reply DoS, consent-bypass/elevation) and the **structural consent gap (finding #2)** are accepted **only if** the conditions land; the **WhatsApp cost ceiling** and the **responsible-party ruling** are **escalated**, not solved in code.

### Conditions baked into the frozen M3-B contract (testable)
...
7. **Loop / cost-DoS prevention.** Fire ONLY when the just-persisted `Message` is `direction = INBOUND` AND `type ∈ {TEXT, INTERACTIVE}`; enforce `cooldownMinutes`/once-per-period from the prior OUTBOUND auto-reply of the same `trigger`.
...
**Gate: APPROVED-WITH-CONDITIONS — freeze unblocked once Conditions 1-9 are folded into the frozen contract and R1 is consciously documented per Condition 5.**
